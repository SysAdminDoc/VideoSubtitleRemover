...
- [ ] P1 -- RM-160: The entire screen-reader announcement layer has never worked on any machine
  ...
  Problem: `CUIAutomation8` is a coclass name, not a registered ProgId, so `CreateObject` cannot resolve it -- `CoCreateInstance` needs the CLSID `{E22AD333-B25F-460C-83D0-0581107395C9}` (or `comtypes.client.GetModule("UIAutomationCore.dll")` and the generated coclass). The call raises, the bare `except` logs at debug level, and `_PROVIDER` stays `None` forever. Every focus announcement wired into ModernButton, ModernToggle, ModernSlider, the segmented picker, the drop target, queue rows, `show_confirm`, batch lifecycle events, and `_update_status` is a permanent silent no-op. Compounding it, **`comtypes` is not declared in `requirements.txt`, `setup.py`, or `VideoSubtitleRemoverPro.spec`**, so even a corrected class string could never activate in a standard install or the frozen build. (This is distinct from the UIA *provider* work parked in Roadmap_Blocked.md, which is about exposing Canvas widgets to the tree; this is the notification path, which is pure client-side API use and needs no screen reader to fix.)
  Evidence: Reproduced live with comtypes 1.4.16 on this machine -- `comtypes.client.CreateObject('CUIAutomation8')` raises `OSError [WinError -2147221005] Invalid class string`. `grep -rn "comtypes" requirements.txt setup.py VideoSubtitleRemoverPro.spec` returns nothing.
  Fix: Instantiate through the UIAutomationClient typelib (`GetModule("UIAutomationCore.dll")` then `CreateObject(CUIAutomation8, interface=IUIAutomation)`) or by explicit CLSID GUID; add `comtypes` to `requirements.txt`, the setup installer, and the spec's `hiddenimports`; and log the probe failure at INFO rather than debug so a broken provider is visible in the log panel instead of invisible.
  Acceptance: `backend.a11y._probe_provider()` returns a live provider on this machine, a startup log line states the provider is ready, and a unit test asserts the probe does not swallow an `Invalid class string` failure silently.
...
- [ ] P2 -- RM-166: Windows taskbar progress has never worked -- the COM call raises on every launch
  Category: correctness
  Where: `gui/widgets.py:1346-1354` (`TaskbarProgress.__init__`)
  Problem: `comtypes.client.CreateObject(clsid, interface=comtypes.GUID("{EA1AFB91-...}"))` passes a `GUID` *instance* where comtypes requires an interface *class*, raising `TypeError: must be a ctypes type`. The blanket `except` sets `self._taskbar = None`, so `set_value` and `set_state` are permanent no-ops. The feature shipped in 3.7.0 and has been dead since. It is additionally blocked by the missing `comtypes` dependency described in RM-160.
  Evidence: Reproduced live on this machine with comtypes 1.4.16 -- the exact call from `widgets.py:1348-1351` raises `TypeError: must be a ctypes type`.
  Fix: Obtain a real `ITaskbarList3` interface class (via `comtypes.client.GetModule` on the shell typelib, or a hand-declared `comtypes.IUnknown` subclass with the correct vtable order) and pass that class as `interface=`. Add `comtypes` to the dependency set and the spec hidden imports alongside the RM-160 fix, and log the failure rather than swallowing it.
  Acceptance: Starting a batch drives the Windows taskbar button progress; the constructor logs a specific reason when the provider is unavailable instead of failing silently.
  Confidence: Verified (reproduced empirically)
  Effort: M
