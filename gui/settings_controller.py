...
class AdvancedSettingsControllerMixin:
        ...
        if section is not None:
            try:
                self.root.after_idle(
                    lambda selected=section: self._reveal_inspector_section(
                        selected))
            except tk.TclError:
                pass
    ...
    def _export_settings_copy_dialog(self):
        ...
        try:
            ...
            self._update_status(
                tr("Export failed: {error}").format(error=exc), "error")
    ...
    def _export_preset_dialog(self):
        ...
        try:
            ...
            if export_preset(name, path):
                self._update_status(
                    tr("Exported '{name}' to {file}").format(
    ...
    def _import_preset_dialog(self):
        ...
        try:
            ...
            if notice:
                self._update_status(
                    tr("Imported preset '{name}'. {notice}").format(
                        name=new_name, notice=notice),
                    "warning",
                )
            else:
                self._update_status(
                    tr("Imported preset '{name}'").format(name=new_name),
                    "success")
        except Exception as exc:
            self._update_status(
                tr("Import failed: {error}").format(error=exc), "error")
    ...
    def _save_preset_dialog(self):
        ...
        try:
            ...
            if save_user_preset(name, description, self.config):
                ...
                self._update_status(
                    tr("{verb} preset '{name}'").format(verb=verb, name=name),
                    "success")
            else:
                self._update_status(
                    tr("Could not save preset '{name}'").format(name=name),
                    "error")
        except Exception as exc:
            self._update_status(
    ...
    def _on_gpu_changed(self, event=None):
        ...
        for i, gpu in enumerate(self.gpus):
            ...
            if label == selection:
                ...
                self._update_status(
                    tr("Compute device set to {device}").format(
                        device=gpu["name"]),
                    "info")
