...
- LaMa can be obtained from inside the product. Every LaMa backend needs a weight file the installer does not ship, and there was no way to get one: the ONNX and OpenCV DNN paths waited on an environment variable, and the PyTorch rung is excluded from the frozen build, so choosing LaMa in a release build failed for everyone who tried it. `--list-fetchable-models` names what is available and `--fetch-model opencv-lama` downloads it, from a pinned repository and commit, over one bounded HTTPS request, checking the SHA-256 against the shipped manifest before the file is usable. A cancelled, failed, truncated or substituted download leaves nothing behind rather than a broken model that fails hours into a render. Nothing is ever fetched without being asked for, and the failure message now names the command instead of describing a file to go and find.
