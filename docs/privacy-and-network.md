...
| LaMa ONNX weights | You run `--fetch-model opencv-lama` or `--fetch-model lama-onnx`, or accept the offer in the interface. Never automatic. | `huggingface.co/opencv/inpainting_lama` and `huggingface.co/Carve/LaMa-ONNX`, both at a pinned commit | VSR app model cache under `models/` | VSR pins the repository, the full commit, and the file SHA-256. A mismatch deletes the download instead of using it. Offline alternative: copy the same file in by hand and point `VSR_OPENCV_LAMA` or `VSR_LAMA_ONNX` at it, or use STTN, which needs no weight. `VSR_MODEL_ENDPOINT` redirects the fetch at a mirror. |
